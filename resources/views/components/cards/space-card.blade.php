...
<x-ui.card :hover="true">
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center justify-between pt-4 border-t">
            <div class="space-y-1">
                @if($hourlyRate)
                    <div class="text-lg font-bold text-primary">{{ $hourlyRate }} <span class="text-sm font-normal text-muted-foreground">/hour</span></div>
                @endif
                @if($dailyRate)
                    <div class="text-sm text-muted-foreground">or {{ $dailyRate }}/day</div>
                @endif
            </div>
